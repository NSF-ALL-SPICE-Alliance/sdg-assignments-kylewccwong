...
```{r}
k2 <- kmeans(sdr_data_imputed, centers = 25)
```
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  ...
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Unemployment",
       subtitle = "Top 10",
       y = "SDG Indicator",
...
```{r}
pdp::partial(rf_unemp, pred.var = "normalized_score_sdg4_earlyedu", plot = TRUE)
```
...
```{r}
pd <- pdp::partial(rf_unemp, pred.var = c("normalized_score_sdg4_earlyedu", "normalized_score_sdg5_lfpr"))
# Default PDP
plotPartial(pd)
```
